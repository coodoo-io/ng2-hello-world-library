  ...
  webpack(
      {
        ...
        devtool: 'source-map',
        externals: [
          {'@angular/core': ngExternal('core'), '@angular/common': ngExternal('common'), '@angular/forms': ngExternal('forms')},
          rxjsExternal
        ]
      },
...
gulp.task('npm', function() {
  ...
  targetPkgJson['name'] = 'ng2-hello-world-library';
  ...
  return gulp.src('README.md').pipe(gulpFile('package.json', JSON.stringify(targetPkgJson, null, 2))).pipe(gulp.dest('dist'));
});
...
function startKarmaServer(isTddMode, isSaucelabs, done) {
  ...
  if (isSaucelabs) {
    ...
    config['browsers'] = ['SL_CHROME', 'SL_FIREFOX', 'SL_IE9', 'SL_IE10', 'SL_IE11', 'SL_EDGE13', 'SL_EDGE14', 'SL_SAFARI9', 'SL_SAFARI10'];
...
gulp.task('ddescribe-iit', function() { return gulp.src(PATHS.specs).pipe(ddescribeIit({allowDisabledTests: false})); });

gulp.task('test', ['build:tests'], function() {
  startKarmaServer(false, false, () => { return gulp.src(PATHS.coverageJson).pipe(remapIstanbul({reports: {'html': 'coverage/html'}})); });
});

gulp.task('remap-coverage', function() { return gulp.src(PATHS.coverageJson).pipe(remapIstanbul({reports: {'html': 'coverage/html'}})); });

gulp.task('tdd', ['clean:tests'], (cb) => {
  var executable = path.join(__dirname, platformPath('/node_modules/.bin/tsc'));
  var startedKarma = false;

  exec(`${executable} -w`, (e) => { cb(e && e.signal !== 'SIGINT' ? e : undefined); }).stdout.on('data', function(data) {
...
function doCheckFormat() {
  return gulp.src(['gulpfile.js', 'karma-test-shim.js', 'misc/api-doc.js', 'misc/api-doc.spec.js', 'misc/demo-gen.js', PATHS.src])
...
gulp.task('build', function(done) { runSequence('lint', 'enforce-format', 'ddescribe-iit', 'clean:build', 'ngc', 'npm', done); });

gulp.task('deploy-demo', function(done) { runSequence('clean:demo', 'build:demo', 'demo-push', 'clean:demo-cache', done); });
